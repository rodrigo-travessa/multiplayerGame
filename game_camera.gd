...
const NOISE_GROWTH : float = 500
const DECAY_RATE : float = 10
...
func _process(delta: float) -> void:
	...
	offset = Vector2(offset_sample_x, offset_sample_y) * shake_strength * current_shake_percentage
	current_shake_percentage = max(current_shake_percentage - (DECAY_RATE * delta), 0)
